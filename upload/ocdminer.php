    ...
    foreach ($df as $z => $X) {
        foreach ($X as $jd => $W) {
            ...
            if (is_array($W)) {
                ...
                $df[$z][stripslashes($jd)] = ($tc ? $W : stripslashes($W));
            }
        }
    }
    return $df;
...
if (isset($_SERVER["HTTP_X_FORWARDED_PREFIX"]) && $_SERVER["HTTP_X_FORWARDED_PREFIX"]) $_SERVER["REQUEST_URI"] = $_SERVER["HTTP_X_FORWARDED_PREFIX"] . $_SERVER["REQUEST_URI"];
$ba = (isset($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] && strcasecmp($_SERVER["HTTPS"], "off")) || ini_bool("session.cookie_secure");
...
if (extension_loaded('pdo')) {
    class Min_PDO extends PDO {
        ...
        public function query(string $I, ?int $Ug = null, ...$args): PDOStatement|false {
            $J = parent::query($I, $Ug, ...$args);
            $this->error = "";
            if (!$J) {
